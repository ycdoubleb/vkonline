...
class SearchVideoAction extends BaseAction {
    ...
    public function run() {
        ...
        $teacher_id = ArrayHelper::getValue($post, 'teacher_id', null);         //老师ID
        $mts_status = ArrayHelper::getValue($post, 'mts_status', null);         //转码状态
        ...
        $query->leftJoin(['TagRef' => TagRef::tableName()], '(TagRef.object_id = Video.id AND TagRef.is_del = 0)');
        $query->leftJoin(['Tags' => Tags::tableName()], 'Tags.id = TagRef.tag_id');
        $query->leftJoin(['Teacher' => Teacher::tableName()], "Video.teacher_id = Teacher.id");
        $query->leftJoin(['User' => User::tableName()], "Video.created_by = User.id");
        //-----------------------
        // 字段过滤
        //-----------------------
        $query->addSelect([
            'Video.id as id', 'Video.customer_id', 'Video.user_cat_id', 'Video.name', 'Video.img thumb_path',
            'Video.mts_status', 'Video.duration', 'Video.created_at', 'Video.updated_at',
        ...
        // 关键字过滤
        //-----------------------
        $query->having(['or', ['like', 'tags', $keyword], ['like', 'Video.name', $keyword]]);
        //-----------------------
        // 限制数量、计算总数
        //-----------------------
