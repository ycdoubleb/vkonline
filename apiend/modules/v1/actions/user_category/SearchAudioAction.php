...
class SearchAudioAction extends BaseAction {
    ...
    public function run() {
        ...
        $query->leftJoin(['TagRef' => TagRef::tableName()], '(TagRef.object_id = Audio.id AND TagRef.is_del = 0)');
        $query->leftJoin(['Tags' => Tags::tableName()], 'Tags.id = TagRef.tag_id');
        $query->leftJoin(['User' => User::tableName()], "Audio.created_by = User.id");
        //-----------------------
        // 字段过滤
        //-----------------------
        $query->addSelect([
        ...
        // 关键字过滤
        //-----------------------
        $query->having(['or', ['like', 'tags', $keyword], ['like', 'Audio.name', $keyword]]);
        //-----------------------
        // 限制数量、计算总数
        //-----------------------
