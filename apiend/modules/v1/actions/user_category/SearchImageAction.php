...
class SearchImageAction extends BaseActioin{
    public function run(){
        ...
        /* @var $query Query */
        $query = new Query();
        $query->from(['Image' => Image::tableName()]);
        $query->leftJoin(['UserCategory' => UserCategory::tableName()], 'UserCategory.id = Image.user_cat_id');
        $query->leftJoin(['TagRef' => TagRef::tableName()], '(TagRef.object_id = Image.id AND TagRef.is_del = 0)');
        $query->leftJoin(['Tags' => Tags::tableName()], 'Tags.id = TagRef.tag_id');
        $query->leftJoin(['User' => User::tableName()], "Image.created_by = User.id");
        //-----------------------
        // 字段过滤
        //-----------------------
        $query->addSelect([
        ...
        if(!$recursive){
            //只在当前目录下搜索
            $query->andWhere(['Image.user_cat_id' => $user_cat_id]);
        }else{
            //递归搜索所有目录
            $query->andWhere(['or',
        ...
        // 关键字过滤
        //-----------------------
        $query->having(['or', ['like', 'tags', $keyword], ['like', 'Image.name', $keyword]]);
        //-----------------------
        // 限制数量、计算总数
        //-----------------------
