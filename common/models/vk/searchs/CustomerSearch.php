...
class CustomerSearch extends Customer {
    ...
    public function rules() {
        return [
            ...
            [['expire_time', 'renew_time', 'good_id', 'province', 'city', 'district', 'twon', 'address', 'is_official', 'level',
    ...
    public function search($params) {
        ...
        self::$query->leftJoin(['CustomerAdmin' => CustomerAdmin::tableName()], 'CustomerAdmin.customer_id = Customer.id AND CustomerAdmin.level = 1'); //关联查询管理员
    ...
    public function searchResources($id) {
        ...
        //获取上个月的起始时间戳和结束时间戳
        $preYear = date('Y', strtotime('-1 month'));
        $preMonth = date('m', strtotime(-date('d') . 'day'));
        ...
        $totalData = ArrayHelper::merge(ArrayHelper::index($totleCourse->asArray()->all(), 'customer_id'), ArrayHelper::index($totalVideo->asArray()->all(), 'customer_id'), ArrayHelper::index($totalPlay->all(), 'customer_id'));
        if (count($totalData)) {
        ...
        $thisMonthData = ArrayHelper::merge(ArrayHelper::index($thisMonthCourse->asArray()->all(), 'customer_id'), ArrayHelper::index($thisMonthVideo->asArray()->all(), 'customer_id'), ArrayHelper::index($thisMonthPlay->all(), 'customer_id'));
        ...
        $lastMonthData = ArrayHelper::merge(ArrayHelper::index($lastMonthCourse->asArray()->all(), 'customer_id'), ArrayHelper::index($lastMonthVideo->asArray()->all(), 'customer_id'), ArrayHelper::index($lastMonthPlay->all(), 'customer_id'));
