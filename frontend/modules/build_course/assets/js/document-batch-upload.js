...
(function (win, $) {
    ...
    var DocumentData = function (id,data) {
        ...
        this.file_id = null;                                //文档文件ID,校检后设置
        ...
        if (!vd || (vd.submit_status == 2 && vd.submit_result)) {
            ...
            if (vd.validate()) {
                ...
                $.post(this.config['add_document_url'], postData, function (response) {
                    ...
                    $(_self).trigger('submitCompleted', vd);         //发送单个文档上传完成
                    _self.__submitNext();
                });
            } else {
                this.__submitNext();
    ...
    DocumentBatchUpload.prototype.setFiles = function (files) {
        ...
        $.each(this.documents, function(){
            if(!fileNameMap[this.document_filename]){
                fileNameMap[this.document_filename] = 1;
            }else{
                fileNameMap[this.document_filename]++;
            }
        });
        $.each(this.documents,function(){
            /* 不在文件列表里将设置为null */
            ...
            /* DocumentData */
            this.setFile(fileNameMap[this.document_filename] > 1 ? [0, 0] : _self.__getFileByName(this.document_filename));
        });
    };
