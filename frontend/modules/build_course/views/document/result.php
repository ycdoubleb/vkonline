...
<div class="document-index vk-material main">
    ...
    <div class="vk-tabs">
        <ul class="list-unstyled pull-left">
            <li>
                <span class="summary">
                    搜索结果： 共搜索到 <b><?= $totalCount ?></b> 个文档素材 
...
<<<JS
    ...
    $('.times-close').click(function(){
        $(location).attr({'href': "../document/index?user_cat_id={$userCatId}"});
    });
        
    //单击整理视频
    $("#arrange").click(function(){
        $(".vk-tabs .pull-right").removeClass("hidden");
        $('input[name="Document[id]"]').removeClass("hidden").prop("checked", false);
    });
        
    //单击取消
    $("#cancel").click(function(){
        $(".vk-tabs .pull-right").addClass("hidden");
        $('input[name="Document[id]"]').addClass("hidden").prop("checked", false);
    });
        
    //单击全选
    $("#allChecked").click(function(){
        $('input[name="Document[id]"]').prop("checked", true);
    });
        
    //单机全不选
    $("#noAllChecked").click(function(){
        $('input[name="Document[id]"]').prop("checked", false);
    });
    ...
    window.showCatalogModal = function(_this){
        var checkObject = $("input[name='Document[id]']");  
        var val = [];
    ...
    $('.vk-table > tbody > tr').each(function(){
        ...
        $(this).click(function(event){
            ...
            var a = $('<a href="../document/view?id=' + key + '"' + 'target="_blank" />').get(0);
