...
<div class="image-form vk-form set-bottom">
    ...
    ])->label(Yii::t('app', '{Image}{Des}', [
        'Image' => Yii::t('app', 'Image'), 'Des' => Yii::t('app', 'Des')
    ])) ?>

    <!--图像文件-->
    <div class="form-group field-imagefile-file_id">
        <?= Html::label(Yii::t('app', '{Image}{File}', [
            'Image' => Yii::t('app', 'Image'), 'File' => Yii::t('app', 'File')
...
$js = <<<JS
    ...
    require(['euploader'], function (euploader) {
        ...
        window.config = {
            ...
            formData: {
                ...
                //指定文件上传到的应用
                app_id: "$app_id",
                replace_id: imageFiles.length > 0 ? imageFiles[0]['id'] : '',
                //同时创建缩略图
                makeThumb: 1
            }
        ...
        //音频
        window.uploader = new euploader.Uploader(window.config, euploader.TileView);
        window.uploader.clearAll();
        window.uploader.addCompleteFiles(imageFiles);
    });
    /**
    * 上传文件完成才可以提交
    ...
    */
     ...
     */
    function isExist(){
        var len = $('#uploader-container input[name="'+ 'ImageFile[file_id][]'+'"]').length;
        if(len <= 0){
            return false;
        }else{
            return true;
